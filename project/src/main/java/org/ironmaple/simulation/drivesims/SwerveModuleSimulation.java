...
public class SwerveModuleSimulation {
    ...
    public Vector2 updateSimulationSubTickGetModuleForce(
        ...
        // Delegate to overloaded method with global dt for backwards compatibility
        return updateSimulationSubTickGetModuleForce(
                moduleCurrentGroundVelocityWorldRelative,
                robotFacing,
                gravityForceOnModuleNewtons,
     ...
     *
     *
     * <h2>Updates the Simulation for This Module with explicit time step.</h2>
     *
     * <p><strong>Note:</strong> Use this method when running on a threaded physics engine where the physics tick rate
     * differs from the command loop rate. This ensures consistent integration regardless of tick frequency.
     *
     * @param moduleCurrentGroundVelocityWorldRelative the current ground velocity of the module, relative to the world
     * @param robotFacing the absolute facing of the robot, relative to the world
     * @param gravityForceOnModuleNewtons the gravitational force acting on this module, in newtons
     * @param dt the time step for this physics tick (use physics thread's dt, not global)
     * @return the propelling force generated by the module, as a {@link Vector2} object
     */
    public Vector2 updateSimulationSubTickGetModuleForce(
            Vector2 moduleCurrentGroundVelocityWorldRelative,
            ...
            double gravityForceOnModuleNewtons,
            Time dt) {
        /* Step1: Update the steer mechanism simulation with explicit dt */
        steerMotorSim.update(dt);
        ...
        /*
         * Step3: Updates and caches the encoder readings for odometry simulation with
         * explicit dt
         */
        updateEncoderCaches(dt);
    ...
    private void updateEncoderCaches() {
        updateEncoderCaches(SimulatedArena.getSimulationDt());
    }

    /**
     *
     *
     * <h2>Cache the encoder values for high-frequency odometry with explicit dt.</h2>
     *
     * <p>An internal method to cache the encoder values to their queues using a specific time step.
     *
     * @param dt the time step for integration
     */
    private void updateEncoderCaches(Time dt) {
        /* Increment of drive wheel position using explicit dt */
        this.driveWheelFinalPosition = this.driveWheelFinalPosition.plus(this.driveWheelFinalSpeed.times(dt));
